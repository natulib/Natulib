...
> 🔵 **Divergence connue** — Le driver `bme280` de Natulib OS ne lit pas de champ `address` : au démarrage, il essaie automatiquement l'adresse `0x76`, puis `0x77` en cas d'échec. Le câblage `SDO → GND` recommandé ci-dessous (adresse `0x76`) reste pertinent, mais un champ `"address"` ajouté au JSON serait actuellement ignoré. Voir **[Drivers](../natulib-os/drivers.md)**.
